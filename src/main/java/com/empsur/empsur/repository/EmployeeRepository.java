...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query(
        value = "select distinct employee from Employee employee left join fetch employee.user left join fetch employee.company left join fetch employee.department",
    ...
    @Query(
        "select distinct employee from Employee employee left join fetch employee.user left join fetch employee.company left join fetch employee.department"
    )
    List<Employee> findAllWithToOneRelationships();

    @Query(
        "select employee from Employee employee left join fetch employee.user left join fetch employee.company left join fetch employee.department where employee.id =:id"
    )
